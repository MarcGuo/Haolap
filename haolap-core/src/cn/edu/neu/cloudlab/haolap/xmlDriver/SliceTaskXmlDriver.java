...
public class SliceTaskXmlDriver implements OLAPTaskXmlDriver {
    private final static String sliceXmlKey = SystemConf.getSliceTaskXmlKey();
    ...
    public SliceTaskXmlDriver(String xmlStr) throws DocumentException,
            XmlTypeErrorException {
        super();
        if (!isSliceTaskXml(xmlStr)) {
            throw new XmlTypeErrorException("not a valid sliceTask Xml");
        }
    ...
    public static boolean isSliceTaskXml(String xmlStr) {
        xmlStr = xmlStr.toLowerCase().replace("\n", "");
        String regex = "<type>" + sliceXmlKey + "</type>";
